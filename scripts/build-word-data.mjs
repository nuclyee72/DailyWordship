 ...
 *  - guesses-{2,3,4}.txt  추측 허용 사전. 위 세 출처 전부 ∪ 출제 풀
 */
...
async function main() {
  ...
  const [commonSrc, allSrc, oktSrc] = await Promise.all([fetchText(SRC.common), fetchText(SRC.all), fetchText(SRC.okt)]);
  ...
  for (const len of LENGTHS) {
    ...
    const guesses = new Set([...common, ...all, ...okt].filter((w) => isWordOfLen(w, len)));
  ...
  console.log('저장 완료: src/data/answers-*.txt, src/data/guesses-*.txt');
}
