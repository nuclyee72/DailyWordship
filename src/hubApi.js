...
import { summarize, loadProgress, DIST_BUCKETS } from './daily/storage.js';
import { buildCalendarShareText, buildShareText } from './daily/share.js';
import { computeState } from './game/game.js';
...
export function stats(mode) {
  ...
  return {
    ...
    distTitle: '사용한 추측 수',
    dist: DIST_BUCKETS.map((label, i) => ({ label, count: s.distribution[i], fail: i === DIST_BUCKETS.length - 1 })),
  };
}
